...
export function FleetProvider({ children }: { children: ReactNode }) {
  // computeFleet is synchronous (static imports), so run it in a lazy state
  // initializer: data is ready on first render with no mount-time flash, and a
  // thrown wiring error becomes the error state instead of crashing.
  const [state] = useState<ProviderState>(() => {
    try {
      return { status: 'ready', data: computeFleet() }
    } catch (err) {
      return {
        status: 'error',
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-6">
        <div className="max-w-md rounded-lg border border-slate-200 bg-white px-6 py-5 text-center">
          ...
          <p className="mt-1 font-mono text-xs text-slate-500">{state.message}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <FleetContext.Provider value={state.data}>{children}</FleetContext.Provider>
  )
}
